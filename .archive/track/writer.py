...
def resolve_table_writing(content_dict: dict):
    ...
    for i_name in icon_names:
        ...
        if not resolved_path.is_symlink():
            is_symlink = f'<img src="./python/track-all-in-one/check.svg" width={checkmark_width}>'
        else:
            is_symlink = f'<img src="./python/track-all-in-one/ban.svg" width={checkmark_width}>'
